 ...
 */
export function LuciditvListTile(
...
const ListTileImpl = React.forwardRef<HTMLButtonElement, Props>(
  function ListTileImpl(
    ...
    const getClassName = getClassNamesFor('LuciditvListTile', moduleClassName);
    ...
    const contents = (
      <>
        {leading && <div className="LuciditvListTile__leading">{leading}</div>}
        <div className="LuciditvListTile__content">
          <div className="LuciditvListTile__title">{title}</div>
          {subtitle && (
            <div
              className={classNames(
                'LuciditvListTile__subtitle',
                `LuciditvListTile__subtitle--max-lines-${subtitleMaxLines}`
              )}
            >
              {subtitle}
            </div>
        ...
        {trailing && (
          <div className="LuciditvListTile__trailing">{trailing}</div>
        )}
      </>
    );
...
LuciditvListTile.heightFull = 64;

/** Effective ListTile height for an avatar (leading) size 48 */
LuciditvListTile.heightCompact = 52;
